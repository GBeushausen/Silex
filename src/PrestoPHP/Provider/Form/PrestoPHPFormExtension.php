...
namespace PrestoPHP\Provider\Form;
...
class PrestoPHPFormExtension implements FormExtensionInterface
{
    private $app;
    private $types;
        ...
        foreach ($types as $type) {
            if (!is_object($type)) {
                if (!isset($this->app[$type])) {
                    throw new InvalidArgumentException(sprintf('Invalid form type. The PrestoPHP service "%s" does not exist.', $type));
                }
        ...
        foreach ($typeExtensions as $extension) {
            if (!is_object($extension)) {
                if (!isset($this->app[$extension])) {
                    throw new InvalidArgumentException(sprintf('Invalid form type extension. The PrestoPHP service "%s" does not exist.', $extension));
                }
    ...
    private function setGuessers(array $guessers)
    {
        ...
        foreach ($guessers as $guesser) {
            if (!is_object($guesser) && !isset($this->app[$guesser])) {
                throw new InvalidArgumentException(sprintf('Invalid form type guesser. The PrestoPHP service "%s" does not exist.', $guesser));
            }
